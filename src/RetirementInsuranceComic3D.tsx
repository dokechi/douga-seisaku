...
const Stage = ({children, dark = false}: {children: ReactNode; dark?: boolean}) => {
  ...
  return <AbsoluteFill style={{fontFamily: font, color: dark ? 'white' : C.navy, background: dark ? C.navy : C.paper, overflow: 'hidden'}}>
    ...
    <div style={{position: 'absolute', inset: '105px 68px 90px', perspective: 1400}}>{children}</div>
  </AbsoluteFill>;
};
...
const Hook = ({data}: {data: RetirementInsuranceData}) => <Stage dark><Pop delay={5}><Kicker>{data.hook.eyebrow}</Kicker></Pop><div style={{fontSize: 150, whiteSpace: 'pre-line', fontWeight: 1000, lineHeight: .98, marginTop: 180, letterSpacing: -8}}>{data.hook.title}</div><Panel delay={32} side={1} style={{padding: '35px 38px', marginTop: 120, color: C.navy, fontSize: 41, fontWeight: 850, lineHeight: 1.55, borderColor: C.red}}>{data.hook.sub}</Panel><div style={{position: 'absolute', right: 30, top: 320, fontSize: 260, color: C.red, fontWeight: 1000, transform: 'rotate(12deg)', opacity: .22}}>!</div></Stage>;

const Hospital = ({data}: {data: RetirementInsuranceData}) => <Stage><div style={{display: 'flex', gap: 28, alignItems: 'center'}}><Pop delay={4}><HospitalIcon/></Pop><div style={{fontSize: 59, fontWeight: 950, lineHeight: 1.25}}>{data.hospital.title}</div></div><div style={{display: 'grid', gap: 38, marginTop: 150}}>{data.hospital.facts.map((fact, i) => <Panel key={fact} delay={20 + i * 16} side={i ? 1 : -1} style={{padding: '50px 42px', fontSize: 50, fontWeight: 900}}><span style={{color: C.blue, marginRight: 24}}>✓</span>{fact}</Panel>)}</div><Pop delay={66} style={{marginTop: 145, textAlign: 'center', fontSize: 40, fontWeight: 900, color: C.red}}>{data.hospital.note}</Pop></Stage>;
...
const Pivot = ({data}: {data: RetirementInsuranceData}) => {
  ...
  return <Stage dark><div style={{fontSize: 43, fontWeight: 800, marginTop: 100}}>{data.pivot.lead}</div><div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-around', marginTop: 220}}><div style={{opacity: interpolate(frame, [20, 55], [1, .16], {extrapolateRight: 'clamp'})}}><HospitalIcon/><div style={{fontSize: 57, fontWeight: 900, marginTop: 35}}>{data.pivot.from}</div></div><div style={{fontSize: 85, color: C.red, fontWeight: 1000}}>→</div><div style={{transform: `scale(${interpolate(wipe, [0,100], [.7,1.18])})`}}><CalendarIcon/><div style={{fontSize: 57, fontWeight: 950, marginTop: 35, color: '#fff'}}>{data.pivot.to}</div></div></div><div style={{position: 'absolute', left: -70, right: -70, top: 720, height: `${wipe}%`, maxHeight: 500, background: C.red, transform: 'skewY(-7deg)', zIndex: -1}}/><Pop delay={55} style={{fontSize: 92, fontWeight: 1000, lineHeight: 1.1, marginTop: 250}}>病院代より<br/><span style={{color: '#ffd65a'}}>「休んだ月」</span></Pop></Stage>;
};
...
const Comparison = ({data}: {data: RetirementInsuranceData}) => {
  ...
  return <Stage><div style={{fontSize: 48, fontWeight: 950, lineHeight: 1.3}}>{data.comparison.title}</div><div style={{display: 'flex', gap: 10, margin: '38px 0 50px'}}>{Array.from({length: data.comparison.days}).map((_, i) => <div key={i} style={{height: 24, flex: 1, background: i < visibleDays ? (i > 23 ? C.red : C.blue) : '#d6deea', transform: `translateY(${i % 2 ? 3 : 0}px)`}} />)}</div><div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28}}><CompareCard kind="social" data={data} side={-1}/><CompareCard kind="national" data={data} side={1}/></div><Pop delay={74} style={{marginTop: 65, background: C.navy, color: 'white', padding: '34px', fontSize: 54, fontWeight: 1000, textAlign: 'center'}}>収入の差が、生活の差になる。</Pop><div style={{fontSize: 23, marginTop: 34, lineHeight: 1.5}}>{data.comparison.disclaimer}</div></Stage>;
};

const Checklist = ({data}: {data: RetirementInsuranceData}) => <Stage><Kicker>辞める前に見るのは、この3つ</Kicker><div style={{display: 'grid', gap: 38, marginTop: 120}}>{data.checklist.map((item, i) => <Panel key={item.number} delay={12 + i * 18} side={i % 2 ? 1 : -1} style={{display: 'grid', gridTemplateColumns: '115px 130px 1fr', alignItems: 'center', padding: '38px 34px'}}><div style={{fontSize: 32, fontWeight: 950, color: C.red}}>{item.number}</div><MiniIcon type={item.icon}/><div><div style={{fontSize: 62, fontWeight: 1000}}>{item.title}</div><div style={{fontSize: 29, fontWeight: 700, marginTop: 8}}>{item.detail}</div></div></Panel>)}</div><Pop delay={86} style={{marginTop: 100, display: 'flex', justifyContent: 'center', gap: 35, alignItems: 'center'}}><MoneyIcon/><span style={{fontSize: 46, fontWeight: 950}}>会社が払っていた分も、<br/><span style={{color: C.red}}>退職後は自分ごと。</span></span></Pop></Stage>;

const Conclusion = ({data}: {data: RetirementInsuranceData}) => <Stage dark><Pop delay={5}><ShieldIcon/></Pop><div style={{fontSize: 65, lineHeight: 1.45, fontWeight: 850, whiteSpace: 'pre-line', marginTop: 130}}>{data.conclusion.lead}</div><Pop delay={34} style={{fontSize: 85, color: '#ffd65a', fontWeight: 1000, lineHeight: 1.25, marginTop: 55}}>{data.conclusion.accent}</Pop><Panel delay={66} style={{padding: '48px 30px', marginTop: 170, color: C.navy, textAlign: 'center', fontSize: 44, fontWeight: 1000, borderColor: C.red}}>{data.conclusion.final}</Panel><div style={{position: 'absolute', bottom: 10, right: 0, color: '#8eb9ee', fontSize: 25, fontWeight: 700}}>退職前 CHECK</div></Stage>;
